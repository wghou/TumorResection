...
void DfSurfaceMesh::rendering(RenderableObject* rdFactory)
{
	...
	memcpy(mVertices, m_srcVertPtr, sizeof(float)*m_srcVertNum * 3);
	for (int i = 0; i < m_VertCpy.size(); i = i + 2) {
		uint16_t newIndex = m_VertCpy[i];
		uint16_t fv_v = m_VertCpy[i + 1];
		mVertices[newIndex * 3 + 0] = m_srcVertPtr[fv_v * 3 + 0];
		mVertices[newIndex * 3 + 1] = m_srcVertPtr[fv_v * 3 + 1];
		mVertices[newIndex * 3 + 2] = m_srcVertPtr[fv_v * 3 + 2];
	}
...
void DfSurfaceMesh::setVertCpys(int srcVtNum, float* srcVtPtr, std::vector<uint16_t> &cpy)
{
